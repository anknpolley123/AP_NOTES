...
import { 
  ...
  MousePointer2, Shapes, Table, ListTodo, Brush, Pen, Outdent, Indent, Maximize2, Minimize2, GraduationCap, Calendar
} from 'lucide-react';
...
import { 
  ...
  SummaryOptions, chatWithNote, autoTagNote, generateQuiz, generateStudyPlan, transcribeAudio
...
export default function EditorScreen() {
  ...
  useEffect(() => {
    ...
    if (id) {
      if (isCollaborative) {
        // We use an async inner function for setup
        const startCollab = async () => {
          unsubscribe = await setupCollaboration();
        };
        startCollab();
      } else {
        loadNote();
  ...
  const setupCollaboration = async () => {
    ...
    try {
      ...
      const unsub = onSnapshot(doc(db, 'notes', id), (snapshot) => {
        if (snapshot.exists()) {
          ...
          if (!isCollabInSync) {
             ...
             // Remote change detection - simplified check
  ...
  const handleAutoTag = async () => {
    ...
    setAiStatus('ANALYZING FOR TAGS...');
    try {
      const suggestedTags = await autoTagNote(text);
  ...
  const handleSendChatMessage = async () => {
    ...
    try {
      const response = await chatWithNote(text, userMsg, chatMessages);
      setChatMessages([...newMessages, { role: 'model' as const, text: response }]);
    } catch (e) {
      console.error(e);
      setChatMessages([...newMessages, { role: 'model' as const, text: "Sorry, I couldn't process that. Check your connection." }]);
    } finally {
    ...
    try {
      ...
      switch (action) {
        ...
        case 'actions':
          ...
          break;
        case 'generate_quiz':
          setAiStatus('GENERATING QUIZ...');
          result = await generateQuiz(text);
          updateText(text + "\n\n---\n## 🎓 AI Generated Quiz\n\n" + result);
          break;
        case 'study_plan':
          setAiStatus('CREATING STUDY PLAN...');
...
3. [Wildcard]`
    ...
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        reader.onloadend = async () => {
          ...
          const base64Audio = base64DataWithPrefix.split(',')[1];
          const audioTag = `\n\n<audio controls src="${base64DataWithPrefix}"></audio>\n\n`;
          ...
          try {
            ...
            if (textarea) {
              const start = textarea.selectionStart;
              const end = textarea.selectionEnd;
              const newText = text.substring(0, start) + audioTag + transcriptionBlock + text.substring(end);
              updateText(newText);
            } else {
              updateText(text + audioTag + transcriptionBlock);
            }
          } catch (e) {
            console.error("AI Transcription failed", e);
            // Fallback to just audio tag
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col overflow-hidden">
      {/* Top Header matched to screenshot - Android Optimized */}
      <header className={`bg-white border-b border-slate-100 flex items-center justify-between px-3 sticky top-0 z-[60] shadow-sm ${window.Capacitor ? 'pt-[var(--safe-top)] h-auto pb-2' : 'h-14'}`}>
        <div className="flex items-center gap-2">
          <button onClick={() => navigate(-1)} className="p-2 text-slate-800 hover:bg-slate-50 rounded-full transition-colors">
      ...
      <AnimatePresence>
        {showToolbar && (
          <motion.footer 
            ...
            className={`fixed ${toolbarPosition === 'bottom' ? 'bottom-0' : 'top-14'} inset-x-0 bg-[#f8f9fc]/95 backdrop-blur-xl border-t border-slate-200 z-[60] pb-[var(--safe-bottom)] shadow-2xl transition-all`}
          >
                    ...
                    {showAiMenu && (
                      ...
                      >
                         ...
                         <div className="grid grid-cols-2 gap-2 p-2">
                            ...
                            <button onClick={() => { setShowAiMenu(false); setShowChatSidebar(true); }} className="flex flex-col items-center gap-2 p-4 hover:bg-slate-50 rounded-[24px] group">
                               <div className="w-10 h-10 bg-green-50 rounded-2xl flex items-center justify-center text-green-500 group-hover:scale-110 transition-all"><MessageSquareText className="w-5 h-5" /></div>
                               <span className="text-[9px] font-bold uppercase tracking-widest text-slate-500">AI Chat</span>
                            </button>
                            <button onClick={() => { setShowAiMenu(false); handleAutoTag(); }} className="flex flex-col items-center gap-2 p-4 hover:bg-slate-50 rounded-[24px] group">
                               <div className="w-10 h-10 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-500 group-hover:scale-110 transition-all"><PenLine className="w-5 h-5" /></div>
                               <span className="text-[9px] font-bold uppercase tracking-widest text-slate-500">AI Tags</span>
                            </button>
                            <button onClick={() => { setShowAiMenu(false); handleAiAction('generate_quiz'); }} className="flex flex-col items-center gap-2 p-4 hover:bg-slate-50 rounded-[24px] group">
                               <div className="w-10 h-10 bg-rose-50 rounded-2xl flex items-center justify-center text-rose-500 group-hover:scale-110 transition-all"><GraduationCap className="w-5 h-5" /></div>
                               <span className="text-[9px] font-bold uppercase tracking-widest text-slate-500">AI Quiz</span>
                            </button>
                            <button onClick={() => { setShowAiMenu(false); handleAiAction('study_plan'); }} className="flex flex-col items-center gap-2 p-4 hover:bg-slate-50 rounded-[24px] group">
                               <div className="w-10 h-10 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-500 group-hover:scale-110 transition-all"><Calendar className="w-5 h-5" /></div>
                               <span className="text-[9px] font-bold uppercase tracking-widest text-slate-500">Study Plan</span>
                            </button>
                         </div>
                         <div className="p-2">
      ...
      <footer className={`fixed bottom-0 inset-x-0 bg-[#f8f9fc]/90 backdrop-blur-md border-t border-slate-200 z-[60] shadow-2xl transition-all pb-[var(--safe-bottom)]`}>
        {/* Row 1: Text Options */}
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-900 text-white">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-sm font-black uppercase tracking-widest leading-none">AI Study Partner</h2>
                    <p className="text-[8px] text-blue-300 font-bold uppercase tracking-widest mt-1 opacity-70">Context: Current Note</p>
                  </div>
                </div>
                <button 
                  onClick={() => setShowChatSidebar(false)}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar bg-slate-50/50">
                {chatMessages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center px-8">
                    <div className="w-16 h-16 rounded-[28px] bg-blue-50 flex items-center justify-center mb-6">
                      <MessageSquareText className="w-8 h-8 text-blue-500" />
                    </div>
                    <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Ask anything about your note</h3>
                    <p className="text-xs text-slate-400 mt-2 font-medium">I can explain complex topics, summarize sections, or help you brainstorm new ideas based on what you've written.</p>
                  </div>
                ) : (
                  <>
                    {chatMessages.map((msg, i) => (
                      ...
                      >
                        <div className={`max-w-[85%] rounded-[24px] p-4 text-sm shadow-sm ${
                          msg.role === 'user' 
                    ...
                    {isChatLoading && (
                      ...
                      >
                        <div className="bg-white border border-slate-100 rounded-[24px] rounded-tl-none p-4 shadow-sm flex items-center gap-2">
              ...
              <div className="p-6 border-t border-slate-100 bg-white">
                <div className="relative flex items-center">
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 pl-5 pr-14 text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-all font-medium"
                  />
                  <button 
                    onClick={handleSendChatMessage}
                    disabled={!chatInput.trim() || isChatLoading}
                    className="absolute right-2 w-10 h-10 bg-blue-600 text-white rounded-xl flex items-center justify-center hover:bg-blue-700 transition-all active:scale-95 disabled:opacity-50"
                  >
